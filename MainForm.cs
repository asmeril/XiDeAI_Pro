...
";
        ...
        private async Task RefreshTrendsAsync()
        {
            ...
            // 0. Get Hard Data (XU100 Real-Time Status) from iDeal scanner logs
            try
            {
                string marketStatFile = @"C:\iDeal\TARAMA_LOG\Market_Status.txt";
                if (System.IO.File.Exists(marketStatFile))
                {
                    string[] statData = System.IO.File.ReadAllText(marketStatFile).Split('|');
                    if (statData.Length >= 4)
                    {
                        string date = statData[0];
                        string mode = statData[1];
            ...
            var financeKeywords = new[] { "BIST", "BORSA", "XU100", "HISSE", "DOLAR", "ALTIN", "EKONOMI", "FED", "TCMB", "FAIZ", "ENFLASYON", "VIOP", "BANKA" };            foreach(var t in realTrends) 
            {
            ...
            string trendsStr = string.Join(" ", finalList);
            if (!string.IsNullOrEmpty(hardDataInfo))
            {
                trendsStr = $"{hardDataInfo} YATIRIMCI_SOSYAL_ALGI: {trendsStr}";
            }
