    ...
    public class SignalEngine : IModule
    {
        ...
        public async Task ProcessStructuredSignal(SignalData sig)
        {
            try
            {
                ...
                // 1. GLOBAL DEDUPLICATION (Persistent since v3.1.2)
                if (_persistence.IsProcessed(sig.Symbol, sig.Period))
                {
                    OnLog?.Invoke($"⏭️ Mükerrer Sinyal Engellendi: {sig.Symbol} ({sig.Strategy}) [Kalıcı Hafıza]", "Engine");
                    return;
                }
                ...
                // 3.5 WEEKLY THREAD LIMIT (v3.8: Max 1 thread per symbol per week)
                if (_memory != null)
                {
                    int weeklyCount = _memory.GetWeeklyThreadCount(sig.Symbol);
                    if (weeklyCount >= 1)
                    {
                        OnLog?.Invoke($"🛡️ Haftalık Limit: {sig.Symbol} için bu hafta zaten {weeklyCount} thread atılmış, pas geçiliyor.", "Engine");
                        return;
                    }
                }

                // v3.9.3: Record to performance BEFORE deep analysis/tweeting
                // This ensures UI shows the signal even if AI or Twitter fails
                _performance.RecordSignal(sig);
                OnSignalProcessed?.Invoke(sig); // Update UI Grid/KPI
                ...
                string aiContent = "";
                string shortThread = ""; // NEW: Short Thread Format
                if (!string.IsNullOrEmpty(screenshotPath))
                {
                    ...
                    try
                    {
                        OnLog?.Invoke($"🐦 Short Thread oluşturuluyor: {sig.Symbol}", "Engine");
                        
                        // Get last week's successful analysis
                        string lastWeekAnalysis = _gemini.GetLastWeekSuccessfulAnalysis(sig.Symbol);
                        
                        // Get influencer context
                        var influPosts = await _socialIntel.FindInfluencerAnalyses(sig.Symbol, sig.Market);
                        string influencerContext = "";
                        if (influPosts != null && influPosts.Count > 0)
                        {
                            influencerContext = string.Join("\n", influPosts.Take(3).Select(p => $"• @{p.Handle}: {p.Content}"));
                        }
                        
                        shortThread = await _gemini.GenerateShortThreadWithHistory(
                            sig.Symbol,
                            sig.Market,
                            priceContext,
                            aiContent,
                        ...
                        {
                            OnLog?.Invoke($"✅ Short thread hazır: {sig.Symbol} (4 tweet)", "Engine");
                        }
                    }
                    catch (Exception ex)
                    {
                        OnLog?.Invoke($"⚠️ Short thread hatası: {ex.Message}", "Engine");
                    }
                }
                else
                {
