...
namespace XiDeAI_Pro.Services.AI
{
    ...
    public class ModelManager
    {
        ...
        private void InitializeTaskPreferences()
        {
            ...
            { 
                "gemini-2.0-flash-exp",  // 1st: Fast and free
                "gemini-2.5-flash",      // 2nd: Fallback
                "gemini-2.5-pro"         // 3rd: Last resort
            };
            
            // News Analysis - Prioritize Flash for speed and cost
            _taskModelPreferences[TaskType.NewsAnalysis] = new List<string> 
            { 
                "gemini-2.0-flash-exp",  // 1st: Fastest
                "gemini-2.5-flash",      // 2nd: Stable fallback
                "perplexity-sonar"       // 3rd: Search Fallback
            };
            ...
            { 
                "gemini-2.5-flash",      // 1st: Good creativity
                "gemini-2.0-flash-exp",  // 2nd: Fast fallback
            ...
            { 
                "gemini-2.5-flash",      // 1st: Good context understanding
                "gemini-2.0-flash-exp",  // 2nd: Fast fallback
            ...
            { 
                "gemini-2.0-flash-exp",  // 1st: Fast and reliable
                "gemini-2.5-pro",        // 2nd: Premium fallback
                "perplexity-sonar"       // 3rd: Can add context
            };

            // Meta-Teacher Analysis
            _taskModelPreferences[TaskType.MetaTeacherAnalysis] = new List<string>
            {
                "gemini-2.0-flash-exp",  // 1st: Fast and reliable
                "perplexity-sonar",      // 2nd: Fallback if Gemini fails
                "gemini-2.5-pro"         // 3rd: Premium
            };

            // Potential Guru Analysis - Fast categorization
            _taskModelPreferences[TaskType.PotentialGuruAnalysis] = new List<string>
            {
                "gemini-2.0-flash-exp",  // 1st: Fast scan
                "perplexity-sonar"       // 2nd: If we need to check reputation
            };
