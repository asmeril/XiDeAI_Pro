...
async def run():
    try:
        ...
        for i, chunk in enumerate(chunks):
            ...
            # Dump all interactive aria-labels to find the add button
            if i == 0:
                labels = await page.evaluate('''
                    () => Array.from(document.querySelectorAll('[aria-label]')).map(el => el.getAttribute('aria-label'))
                ''')
                print('LABELS:', json.dumps(list(set(labels))))
            
            # 2. Click + (add) button if this is not the last chunk
            if i < len(chunks) - 1:
                print('Clicking + button')
                add_btn = page.locator('div[aria-label=\"Add\"], div[aria-label=\"Ekle\"], [data-testid=\"addTweetButton\"]')
